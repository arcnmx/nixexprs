  ...
  config = {
    system.mutableState.services = {
      ...
      matrix-synapse = {
        ...
        databases.postgresql = mkIf (config.services.matrix-synapse.settings.database.name or config.services.matrix-synapse.database_type == "psycopg2" && config.services.postgresql.enable) [ "matrix-synapse" ];
