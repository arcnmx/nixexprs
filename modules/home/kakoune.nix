...
let
  ...
  '' else "") + (if cfg.plugins != [] then let
    sourceString = s: if s ? kakrc then "source ${s}/${s.kakrc}" else "source ${s}";
  in ''
      # Plugins
  ...
  config = mkIf cfg.enable (mkMerge [
    {
      ...
      xdg.configFile = {
        ...
        nameValuePair "kak/colors/${nameFromURL (toString color)}" { inherit source; }
      ) cfg.colors));
    }
  ]);
}
